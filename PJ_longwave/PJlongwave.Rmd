...
for (i in 1:length(PJG_AF_gf)) {
  ...
  # Read in files
  # for now there are 2 more PJC files
  ...
  in_model <- lm(pjg$Rlong_in ~ pjc$Rlong_in)
  in_coef <- round(coef(in_model), 3) # extract coefficients 
  plot(pjg$Rlong_in ~ pjc$Rlong_in, main=paste(year, "Rlong incoming"),
       ylab=parse(text="Girdle (W/m^2)"), xlab=parse(text="Control (W/m^2)"), pch=18, col='gray')
  #legend("bottomright", legend="obs", pch=18, col='gray')
  abline(in_model, col='red',lwd=2)
  abline(0,1, col='blue',lwd=2)
  legend("bottomright", legend=c("Fit", "1:1 line"),
         lwd=2, col=c('red', 'blue'))
  text(190, 370, paste("Slope =", in_coef[[2]], "\n Int = ", 
                       in_coef[[1]]))
  
  out_model <- lm(pjg$Rlong_out ~ pjc$Rlong_out)
  out_coef <- round(coef(out_model), 3) # extract coefficients 
  plot(pjg$Rlong_out ~ pjc$Rlong_out, main=paste(year, "Rlong outgoing"),
       ylab=parse(text="Girdle (W/m^2)"), xlab=parse(text="Control (W/m^2)"), pch=18, col='gray')
  abline(out_model, col='red', lwd=2)
  abline(0,1, col='blue', lwd=2)
  text(295, 550, paste("Slope =", out_coef[[2]], "\n Int = ", 
                       out_coef[[1]]))
}
...
for (i in 1:length(PJG_AF_gf)) {
  ...
  in_model <- lm(pjg$Rg ~ pjc$Rg)
  in_coef <- round(coef(in_model), 3) # extract coefficients 
  plot(pjg$Rg ~ pjc$Rg, main=paste(year, "Rg incoming"),
       ylab=parse(text="Girdle (W/m^2)"), xlab=parse(text="Control (W/m^2)"), pch=18, col='gray')
  #legend("bottomright", legend="obs", pch=18, col='gray')
  abline(in_model, col='red',lwd=2)
  abline(0,1, col='blue',lwd=2)
  ...
  out_model <- lm(pjg$Rg_out ~ pjc$Rg_out)
  out_coef <- round(coef(out_model), 3) # extract coefficients 
  plot(pjg$Rg_out ~ pjc$Rg_out, main=paste(year, "Rg outgoing"),
       ylab=parse(text="Girdle (W/m^2)"), xlab=parse(text="Control (W/m^2)"), pch=18, col='gray')
  abline(out_model, col='red', lwd=2)
  abline(0,1, col='blue', lwd=2)
  text(55, 185, paste("Slope =", out_coef[[2]], "\n Int = ", 
                       out_coef[[1]]))
}
...
for (i in 1:length(PJG_fall)) {
  ...
  # Some fluxall files are bigger than others
#   if (nrow(pjc) > nrow(pjg)){
#     ds <- merge(pjc, pjg, all.y=T, by=jday)
#     } 
#   else if (nrow(pjg) > nrow(pjc)) {
...
#     }
  ...
  plot(df$t_c ~ df$t_g, main=paste(year, "CNR1 body temp"),
       ylab="Control (^oC)", xlab="Girdle (^oC)", pch=18, col='gray')
